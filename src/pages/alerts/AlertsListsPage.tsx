      ...
      {alerts.length === 0 ? (
        ...
        <div className="grid gap-4 sm:gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {alerts.map((alert) => (
            <div
              key={alert.id}
              className="bg-white shadow-sm rounded-xl p-5 hover:shadow-md transition flex flex-col justify-between"
            >
              {/* Header */}
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2 mb-3">
                ...
                <button
                  ...
                  className={`text-xs px-3 py-1 rounded-lg transition w-fit ${
                    deleting === alert.id
              ...
              {/* Message */}
              <p className="text-sm text-gray-700 mb-4 line-clamp-4 sm:line-clamp-3">
                {alert.message}
              </p>

              {/* Footer */}
              <div className="flex flex-wrap justify-between items-center text-xs text-gray-500 border-t pt-2 gap-2">
